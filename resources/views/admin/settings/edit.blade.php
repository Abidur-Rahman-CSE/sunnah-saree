...
@section('content')
    <form action="{{ route('admin.settings.update') }}" method="POST" class="rounded-lg border border-[#e5ded0] bg-white p-6 shadow-sm">
        ...
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <x-admin.field label="Facebook page link"><input name="facebook_page_link" value="{{ old('facebook_page_link', $settings['facebook_page_link'] ?? 'https://www.facebook.com/sunnah.saree') }}" class="rounded-lg border border-[#ddd4c4] px-4 py-3" placeholder="Facebook page link"></x-admin.field>
            ...
            <div class="rounded-lg border border-[#ead8ba] bg-[#fffaf4] p-4 md:col-span-2">
                <p class="font-serif text-xl font-bold text-[#2f241f]">Home Page Sections</p>
                <div class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    <x-admin.check label="Hero banner"><input type="checkbox" name="home_section_hero_enabled" value="1" @checked(old('home_section_hero_enabled', ($settings['home_section_hero_enabled'] ?? '1') === '1'))></x-admin.check>
                    <x-admin.check label="Sharee type section"><input type="checkbox" name="home_section_sharee_types_enabled" value="1" @checked(old('home_section_sharee_types_enabled', ($settings['home_section_sharee_types_enabled'] ?? '1') === '1'))></x-admin.check>
                    <x-admin.check label="Color section"><input type="checkbox" name="home_section_colors_enabled" value="1" @checked(old('home_section_colors_enabled', ($settings['home_section_colors_enabled'] ?? '1') === '1'))></x-admin.check>
                    <x-admin.check label="Best sellers"><input type="checkbox" name="home_section_best_sellers_enabled" value="1" @checked(old('home_section_best_sellers_enabled', ($settings['home_section_best_sellers_enabled'] ?? '1') === '1'))></x-admin.check>
                    <x-admin.check label="New arrivals"><input type="checkbox" name="home_section_new_arrivals_enabled" value="1" @checked(old('home_section_new_arrivals_enabled', ($settings['home_section_new_arrivals_enabled'] ?? '1') === '1'))></x-admin.check>
