...
@section('content')
    <form action="{{ $banner->exists ? route('admin.banners.update', $banner) : route('admin.banners.store') }}" method="POST" enctype="multipart/form-data" class="rounded-lg border border-[#e5ded0] bg-white p-6 shadow-sm">
        ...
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <label class="rounded-lg border border-dashed border-[#cfc3ad] px-4 py-3 text-sm md:col-span-2">Upload banner image<input type="file" name="image_file" accept="image/*" data-image-preview="banner-preview" class="mt-2 block w-full"></label>
            <div class="grid gap-2 md:col-span-2">
                ...
                <div id="banner-preview" class="grid grid-cols-4 gap-2 md:grid-cols-6">
                    @if ($banner->image_url)
                        <img src="{{ $banner->image_url }}" alt="{{ $banner->title }}" class="aspect-square rounded-lg border border-[#eadcc3] object-cover">
                    @endif
                </div>
            </div>
